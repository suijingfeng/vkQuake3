...
void vk_imp_init() {
	...
	if (get_render_api() == RENDER_API_VK) {
		g_wv.window = create_main_window(glConfig.vidWidth, glConfig.vidHeight, (qboolean)r_fullscreen->integer);
		g_wv.hWnd = g_wv.window;
		SetForegroundWindow(g_wv.hWnd);
		SetFocus(g_wv.hWnd);
		WG_CheckHardwareGamma();
	} else {
		g_wv.window = create_twin_window(glConfig.vidWidth, glConfig.vidHeight, RENDER_API_VK);
	}
}
